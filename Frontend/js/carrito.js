...
function init() {
    ...
    document.getElementById('btnTramitar')?.addEventListener('click', () => {
        window.location.href = 'registro.html?returnTo=carrito'
    })

    if (!estaAutenticado() || getUsuario()?.rol !== 'consumidor') return
...
function renderCarrito() {
    ...
    // Checkout visible solo si está autenticado como consumidor
    const puedeVerCheckout = auth && user?.rol === 'consumidor'
